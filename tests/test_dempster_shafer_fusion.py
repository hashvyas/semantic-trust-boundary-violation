...
# -- Basic mass function invariants -----------------------------------------
m = MassFunction.from_score_confidence(score=1.0, confidence=1.0)
check("Fully-trusted, fully-confident source -> m_A=1.0", abs(m.m_A - 1.0) < 1e-9)
check("... m_not_A=0.0, m_theta=0.0", abs(m.m_not_A) < 1e-9 and abs(m.m_theta) < 1e-9)

m = MassFunction.from_score_confidence(score=0.0, confidence=0.0)
check("Zero confidence source is fully vacuous regardless of score", abs(m.m_theta - 1.0) < 1e-9)

m = MassFunction.vacuous()
check("vacuous() is all-ignorance", m.m_A == 0.0 and m.m_not_A == 0.0 and m.m_theta == 1.0)

# -- Roadmap's own sanity check: combining {belief=0.9, unc=0.1} with itself
#    twice must monotonically increase certainty (shrink m_theta / raise
#    pignistic trust score away from 0.5, toward whichever side both
#    sources agree on). -------------------------------------------------
m1 = MassFunction(m_A=0.9, m_not_A=0.0, m_theta=0.1)
once = combine(m1, m1)
twice = combine(once, m1)

check("Combining {m_A=0.9, m_theta=0.1} with itself: m_theta strictly decreases",
      once.m_theta < m1.m_theta, f"m1.m_theta={m1.m_theta:.4f} -> once.m_theta={once.m_theta:.4f}")
check("Combining a third time: m_theta decreases further (monotonic)",
      twice.m_theta < once.m_theta, f"once.m_theta={once.m_theta:.4f} -> twice.m_theta={twice.m_theta:.4f}")
check("Pignistic trust score increases toward 1.0 as agreeing evidence accumulates",
      twice.pignistic_trust_score() > once.pignistic_trust_score() > m1.pignistic_trust_score())
check("m_not_A stays at 0.0 throughout (no contradicting evidence introduced)",
      abs(once.m_not_A) < 1e-9 and abs(twice.m_not_A) < 1e-9)

# -- Symmetric case: agreeing distrust also compounds -------------------
d1 = MassFunction(m_A=0.0, m_not_A=0.9, m_theta=0.1)
d_once = combine(d1, d1)
check("Combining {m_not_A=0.9} with itself: m_theta strictly decreases",
      d_once.m_theta < d1.m_theta)
check("Pignistic trust score decreases toward 0.0 as agreeing distrust accumulates",
      d_once.pignistic_trust_score() < d1.pignistic_trust_score())
...
#    power" pathology raw Dempster combination suffers from -- a
